  ...
  return (
    <section className="gallery-section">
      <div className="parent">
        {[0, 1, 2].map((offset) => (
          <div key={offset} className={`div${offset + 1}`} onClick={() => openModal((indexBase + offset) % imagenes.length)}>
            <img
              src={getImg(offset).url}
              alt={getImg(offset).titulo}
              className={`image-tile ${fade ? 'fade-in' : ''}`}
              loading="lazy"
            />
          </div>
        ))}
